...
import { RotateCcwIcon, ChevronLeftIcon, ChevronRightIcon, UsersIcon } from './icons';
import Spinner from './Spinner';
import { AnimatePresence, motion } from 'framer-motion';

interface CanvasProps {
  displayImageUrl: string | null;
  twinDisplayImageUrl: string | null;
  isTwinning: boolean;
  onAddTwin: (file: File) => void;
  onStartOver: () => void;
  isLoading: boolean;
...
const Canvas: React.FC<CanvasProps> = ({ displayImageUrl, twinDisplayImageUrl, onStartOver, isLoading, loadingMessage, onSelectPose, poseInstructions, currentPoseIndex, availablePoseKeys, isTwinning, onAddTwin }) => {
  const [isPoseMenuOpen, setIsPoseMenuOpen] = useState(false);
  const twinUploaderRef = useRef<HTMLInputElement>(null);

  const handleAddTwinClick = () => {
    twinUploaderRef.current?.click();
  };

  const handleTwinFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files && e.target.files[0]) {
          onAddTwin(e.target.files[0]);
      }
  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      {/* Action Buttons */}
      <div className="absolute top-4 left-4 z-30 flex flex-col sm:flex-row items-start gap-2">
        <button 
            onClick={onStartOver}
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        {isTwinning ? (
            <div className="w-full h-full grid grid-cols-2 gap-2 md:gap-4 items-center justify-center">
              <div className="relative w-full h-full flex items-center justify-center">
                  {displayImageUrl ? (
                    <img key={`twinA-${displayImageUrl}`} src={displayImageUrl} alt="Virtual try-on model one" className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg"/>
                  ) : <Spinner />}
              </div>
              <div className="relative w-full h-full flex items-center justify-center">
                  {twinDisplayImageUrl ? (
                      <img key={`twinB-${twinDisplayImageUrl}`} src={twinDisplayImageUrl} alt="Virtual try-on model two" className="max-w-full max-h-full object-contain transition-opacity duration-500 animate-fade-in rounded-lg"/>
                  ) : <Spinner />}
              </div>
            </div>
        ) : (
